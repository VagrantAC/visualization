...
type MatType = Float32Array;

/**
 * Takes two Matrix3s, a and b, and computes the product in the order
 * that pre-composes b with a.  In other words, the matrix returned will
 */
export function multiply(a: MatType, b: MatType, dst = new Float32Array(9)) {
  const a00 = a[0 * 3 + 0];
  const a01 = a[0 * 3 + 1];
  const a02 = a[0 * 3 + 2];
  const a10 = a[1 * 3 + 0];
 ...
 */
export function identity(dst = new Float32Array(9)) {
  dst[0] = 1;
  dst[1] = 0;
  dst[2] = 0;
  dst[3] = 0;
 ...
 */
export function projection(width: number, height: number, dst = new Float32Array(9)) {
 ...
 */
export function project(m: MatType, width: number, height: number, dst: MatType) {
 ...
 */
export function translation(tx: number, ty: number, dst = new Float32Array(9)) {
  dst[0] = 1;
  dst[1] = 0;
  dst[2] = 0;
  dst[3] = 0;
 ...
 */
export function translate(m: MatType, tx: number, ty: number, dst = new Float32Array(9)) {
 ...
 */
export function rotation(angleInRadians: number, dst = new Float32Array(9)) {
 ...
 */
export function rotate(m: MatType, angleInRadians: number, dst = new Float32Array(9)) {
 ...
 */
export function scaling(sx: number, sy: number, dst = new Float32Array(9)) {
  dst[0] = sx;
  dst[1] = 0;
  dst[2] = 0;
  dst[3] = 0;
 ...
 */
export function scale(m: MatType, sx: number, sy: number, dst: MatType = new Float32Array(9)) {
...
export function transformPoint(m: MatType, v: Float32Array) {
  const v0 = v[0];
  const v1 = v[1];
...
export function inverse(m: MatType, dst = new Float32Array(9)) {
  const m00 = m[0 * 3 + 0];
  const m01 = m[0 * 3 + 1];
  const m02 = m[0 * 3 + 2];
  const m10 = m[1 * 3 + 0];
